...
NNL::NNL(CatOperator *parent, CatOperator *child) : parent(parent), child(child){
	counter = 0;
	storage = {};
}
 ...
 */
bool NNL::dominates(const std::vector<int> &dominator, const std::vector<int> &dominated){
...
void NNL::computeSkylineProduceParallel(){
	...
	parallel_for(std::size_t(0), storage.size(), [this, storage, parent]( std::size_t i ) {
		...
		// the mutex slows down the parallelization
		// if it is not there, the programmer has to make sure that no race condition between threads occurs
...
void NNL::consume(const std::vector<int> &inputTuple){
...
void NNL::set_parent(CatOperator *parent){
	this->parent = parent;
}

// dummy method
std::vector<int> NNL::getNext(){
	return {};
}

...
DNC::DNC(Operator *parent, Operator *child) : parent(parent), child(child){
	storage = {};
}
 ...
 */
bool DNC::dominates(const std::vector<double> &dominator, const std::vector<double> &dominated){
 ...
 */
std::vector<double> DNC::median(std::vector<std::vector<double>> tuples, const int &dimension){
	// Sort the tuples asc according to the given dimension
	std::sort(tuples.begin(), tuples.end(), [dimension](const std::vector<double> &a, const std::vector<double> &b){ // non-parallel sort
 ...
 */
std::pair<std::vector<std::vector<double>>, std::vector<std::vector<double>>> DNC::partition(const std::vector<std::vector<double>> &tuples, const int &dimension, const std::vector<double> &pivot){
 ...
 */
std::vector<std::vector<double>> DNC::mergeBasic(std::vector<std::vector<double>> S_1, const std::vector<std::vector<double>> &S_2, const int &dimension){
	...
	if(S_1.size() == 1){ // trivial case - S_1 has only 1 tuple
		...
		std::sort(S_1.begin(), S_1.end(), [](const std::vector<double> &a, const std::vector<double> &b){ // not parallelized sort
			return a[0] < b[0];
		});
		...
		// not parallelized recursive merging
 ...
 */
std::vector<std::vector<double>> DNC::computeSkyline(const std::vector<std::vector<double>> &M, const int &dimension){
...
void DNC::consume(const std::vector<double> &inputTuple){
...
void DNC::set_parent(Operator *parent){
	this->parent = parent;
}

// dummy
std::vector<double> DNC::getNext(){
	return {};
}

// dummy
void DNC::produceParallel(){
	;
}

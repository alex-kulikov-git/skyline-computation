...
Parallel_DNC::Parallel_DNC(Operator *parent, Operator *child) : parent(parent), child(child){
	storage = {};
}
 ...
 */
bool Parallel_DNC::dominates(const std::vector<double> &dominator, const std::vector<double> &dominated){
 ...
 */
std::vector<double> Parallel_DNC::median(std::vector<std::vector<double>> tuples, const int &dimension){
	// Sort the tuples asc according to the given dimension
	tbb::parallel_sort(tuples.begin(), tuples.end(), [dimension](const std::vector<double> &a, const std::vector<double> &b){ // parallel_sort
 ...
 */
std::pair<std::vector<std::vector<double>>, std::vector<std::vector<double>>> Parallel_DNC::partition(const std::vector<std::vector<double>> &tuples, const int &dimension, const std::vector<double> &pivot){
 ...
 */
std::vector<std::vector<double>> Parallel_DNC::mergeBasic(std::vector<std::vector<double>> S_1, const std::vector<std::vector<double>> &S_2, const int &dimension){
	...
	if(S_1.size() == 1){ // trivial case - S_1 has only 1 tuple
		...
		tbb::parallel_sort(S_1.begin(), S_1.end(), [](const std::vector<double> &a, const std::vector<double> &b){ // parallel_sort
			return a[0] < b[0];
		});
		...
		if(S_1.size() > 1000 && S_2.size() > 1000){
			auto future1 = std::async(std::launch::async, &Parallel_DNC::mergeBasic, this, partitions_dim_1.first, partitions_dim_2.first, dimension); // Thread parallelization
			auto future2 = std::async(std::launch::async, &Parallel_DNC::mergeBasic, this, partitions_dim_1.second, partitions_dim_2.second, dimension);
			result_2 = future2.get();
			result_3 = mergeBasic(partitions_dim_1.first, result_2, dimension-1);
			result_1 = future1.get();
		}
		else{
 ...
 */
std::vector<std::vector<double>> Parallel_DNC::computeSkyline(const std::vector<std::vector<double>> &M, const int &dimension){
...
void Parallel_DNC::consume(const std::vector<double> &inputTuple){
	storage.push_back(inputTuple); // store
}

void Parallel_DNC::produce(){
	child->produce();
	std::vector<std::vector<double>> result = computeSkyline(storage, storage[0].size());
	for(std::size_t i = 0; i < result.size(); i++){
		parent->consume(result[i]);
	}
}

void Parallel_DNC::set_parent(Operator *parent){
	this->parent = parent;
}

// dummy
std::vector<double> Parallel_DNC::getNext(){
	return {};
}

// dummy
void Parallel_DNC::produceParallel(){
	;
}
